...
TEST(GeneratedCode, DeepCloneMessageWithUnknowns) {
  ...
  upb_StringView view;
  while (upb_Message_NextUnknown(UPB_UPCAST(clone), &view, &iter)) {
    const char* cloned_unknown_data = view.data;
    EXPECT_EQ(view.size, len);
    EXPECT_EQ(memcmp(cloned_unknown_data, unknown_data.c_str(), view.size), 0);
  }
